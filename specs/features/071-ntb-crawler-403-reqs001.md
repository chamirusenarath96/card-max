...
## Acceptance Criteria
- [ ] AC1: ZenRows URL for NTB includes js_render=true when shouldUseJsRender(nations_trust_bank) true — unit in crawler/utils/proxyProviders/zenrows.test.ts and zenrows-verify.yml log shows js_render=true
- [ ] AC2: When direct fetch throws 403 or isBlockPage true, NTB retries via orderedProvidersForBank (ZenRows then WebScrapingAPI) and returns valid HTML when provider succeeds — unit with mocked provider.fetchHtml + manual gh workflow run zenrows-verify.yml -f bank=nations_trust_bank shows OK ... chars
- [ ] AC3: When NTB scrapes 0 and previousActiveCounts[nations_trust_bank] >0, detectFailures produces zero_offers (verified in crawler/utils/failureAlerts.test.ts and run 31907638309)
- [ ] AC4: Fixture of current live NTB HTML (listing + at least one campaign table) added and parseCampaignTable parses into promotions — unit crawler/scrapers/ntb.test.ts
- [ ] AC5: npm run type-check and npm run lint pass with no new errors; existing ntb.test.ts passes
