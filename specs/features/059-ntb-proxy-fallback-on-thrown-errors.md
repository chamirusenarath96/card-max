# Feature: NTB Proxy Fallback on Thrown Fetch Errors (059)
...
- Investigating why the Crawlee/Playwright fallback also fails on the current CI
  runner IP — that is a separate, larger investigation (browser fingerprinting /
  proxying raw browser traffic) independent of this HTTP-path fix, and is not
  required to satisfy this spec's acceptance criteria
...
- Once this ships, BOC's #121 (ZenRows 422/429) becomes the more likely blocker
  ...
  is reachable."
- This spec's content was originally drafted by `card-max-spec-writer` on branch
  `claude/crawler-failure-monitoring-b7mv2l` as `057-ntb-proxy-fallback-on-thrown-errors.md`,
  but that branch was never merged to master and its number (057) has since been
  taken by an unrelated merged spec. Renumbered to 059 and landed directly on
  master (spec-only change) by `card-max-implementer` before implementation so
  the spec file exists where the mandatory workflow expects it. The AC6
  (manual/`workflow_dispatch` live-run verification) and the Crawlee/Playwright
  IP-block investigation from the original draft were dropped from scope — both
  require live credentials/manual triggering that an automated implementer run
  cannot verify or complete, per the reduced Out of Scope section above.
